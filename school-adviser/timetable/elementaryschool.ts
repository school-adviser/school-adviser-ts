...
import {ElementaryTimetableParams, ElementaryTimetableType} from "../../@types/timetable/elementaryschool";
 ...
 *   .build();
 */
export class ElementaryTimetableBuilder {
    /**
     * @description API URL
     *
     * @private
     ...
     */
    withParams = (params: ElementaryTimetableParams): ElementaryTimetableBuilder => {
     ...
     * @param year 학년도
     */
    withYear = (year: string): ElementaryTimetableBuilder => {
        this.year = year;
        return this;
    }
     ...
     * @param semester 학기
     */
    withSemester = (semester: string): ElementaryTimetableBuilder => {
     ...
     * @param date 시간표일자
     */
    withDate = (date: string): ElementaryTimetableBuilder => {
        this.date = date;
        return this;
    }
     ...
     * @param grade 학년
     */
    withGrade = (grade: string): ElementaryTimetableBuilder => {
        this.grade = grade;
        return this;
    }
     ...
     * @param class_ 학급명
     */
    withClass = (class_: string): ElementaryTimetableBuilder => {
        this.class = class_;
        return this;
    }
     ...
     * @param period 교시
     */
    withPeriod = (period: string): ElementaryTimetableBuilder => {
        this.period = period;
        return this;
    }
     ...
     * @param from 시간표시작일자
     */
    withFrom = (from: string): ElementaryTimetableBuilder => {
        this.from = from;
        return this;
    }
     ...
     * @param to 시간표종료일자
     */
    withTo = (to: string): ElementaryTimetableBuilder => {
        this.to = to;
        return this;
    }
     ...
     */
    withBetween = (between: [string, string]): ElementaryTimetableBuilder => {
        this.from = between[0];
        this.to = between[1];
        return this;
    }
     ...
     * @param page 페이지 위치
     */
    withPage = (page: number): ElementaryTimetableBuilder => {
        this.page = page;
        return this;
    };
     ...
     */
    withPageSize = (pageSize: number): ElementaryTimetableBuilder => {
     ...
     *   .build();
     *
     * @returns {Promise<<ElementaryTimetableType>[]>} 시간표 정보를 담은 객체입니다.
    ...
    build = async (): Promise<AcademyType[]> => {
        try {
            ...
            data.forEach((timetable: ElementaryTimetableType) => {
